...
for(deltaB in deltavals) {
  ...
  irf$WeightStr <- sprintf("delta=%.3f", deltaB)
  irf$WindowLength <- as.integer(1/deltaB)
  
  irf$gamma <- gamma
  irf$lambda <- lambda
  
  irf$Direction <- "Backward"
  
  irf_all.df <- bind_rows(irf_all.df, irf)
}

# Forward looking windows
gamma <- 0.0 # Zero weight given to backward-looking window
deltaB <- 0.01
for(deltaF in deltavals) {
  irf <- nkmodel(gamma, deltaB, deltaF)
  irf <- as_tibble(irf)
  ...
  irf$WeightStr <- sprintf("delta=%.3f", deltaF)
  irf$WindowLength <- as.integer(1/deltaF)
  
  irf$gamma <- gamma
  irf$lambda <- lambda
  
  irf$Direction <- "Forward"
  
  irf_all.df <- bind_rows(irf_all.df, irf)
}

irf_all.df$WindowStr <- as.factor(irf_all.df$WindowLength)
levs <- sprintf("%s Quarters", levels(irf_all.df$WindowStr))
levels(irf_all.df$WindowStr) <- levs
irf_all.df$WindowStr <- factor(irf_all.df$WindowStr, levels=levs, ordered=TRUE)

irf_all.df$WeightStr <- as.factor(irf_all.df$WeightStr)
irf_all.df$WeightStr <- factor(irf_all.df$WeightStr, levels(irf_all.df$WeightStr))


plotirf <- function(irf_all.df, dirstr) {
  delta_labels <- sprintf("%.3f", deltavals)
  
  titlestr <- sprintf("Impulse Responses With Varying %s-Looking Windows", dirstr)
  irf_all.df %>%
    filter(Variable %in% c("Output Gap", "Inflation", "Federal Funds Rate")) %>%
    filter(Shock=="Cost Shock") %>%
    filter(Direction==dirstr) %>%
    ggplot(aes(x=Period, y=Impact, linetype=WeightStr)) +
    ...
    labs(x="Periods After Shock", y="", title=titlestr, linetype=expression(paste("Values for ", delta, ": "))) +
...
plotirf(dirstr="Forward", irf_all.df)
plotirf(dirstr="Backward", irf_all.df)
